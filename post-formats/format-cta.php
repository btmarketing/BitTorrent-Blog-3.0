<?php
/*
Single Post Template: Single - CTA
This will allow for a post that has a CTA in header
*/
?>

<?php 
$ctamain = get_post_meta( $post->ID, '_btm_cta_main', true );
$ctasecond = get_post_meta( $post->ID, '_btm_cta_secondary', true );
$btn1text = get_post_meta( $post->ID, '_btm_cta_btn1_text', true );
$btn1url = get_post_meta( $post->ID, '_btm_cta_btn1_url', true );
$btn2text = get_post_meta( $post->ID, '_btm_cta_btn2_text', true );
$btn2url = get_post_meta( $post->ID, '_btm_cta_btn2_url', true );
$ctainputcolor = get_post_meta( $post->ID, '_btm_cta_input_color', true );
$ctabuttoncolor = get_post_meta( $post->ID, '_btm_cta_button_color', true );
			...
			<div id="content" class="clearfix">
				...
				<div id="main" class="single-main clearfix" role="main">
					...
					<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'featured-test' ); ?>
							<div class="single-featured single-cta" style="background-image: url('<?php echo $image[0]; ?>'); color: <?php echo $ctacolor ?>">
                <h2><?php echo $ctamain ?></h2>
                <div class="row">
                  <div class="col-sm-12">
                    <h4><?php echo $ctasecond ?></h4>
                  </div>
                </div>
              <?php if ($btn2text != '') { ?>
                ...
                <?php } else { ?>
                <div class="button">
                  <a href="<?php echo $btn1url ?>" target="_blank"><button type="button" class="btn btn-bt"><?php echo $btn1text ?></button></a>
                </div>
                <?php } ?>
							</div>
